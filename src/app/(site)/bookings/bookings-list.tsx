"use client";

import { useMemo, useState } from "react";
import { PropertyThumbnail } from "@/components/media/property-thumbnail";
import { BookingStatusBadge, PaymentStatusBadge } from "@/components/ui/badge";
import { Button, ButtonLink } from "@/components/ui/button";
import { EmptyState } from "@/components/feedback/empty-state";
import { ConfirmDialog } from "@/components/feedback/confirm-dialog";
import type { GuestBookingDto } from "@/server/dto/public.dto";
import { formatCurrency, formatDateRange } from "@/lib/utils";
import { useToast } from "@/components/providers/toast-provider";
import { cancelBookingAction } from "@/app/actions/booking.actions";
import { useRouter } from "next/navigation";

type Tab = "upcoming" | "completed" | "cancelled";

function partition(bookings: GuestBookingDto[]) {
  return {
    upcoming: bookings.filter((b) =>
      ["inquiry", "reserved", "payment_pending", "confirmed", "checked_in"].includes(b.bookingStatus),
    ),
    completed: bookings.filter((b) => b.bookingStatus === "checked_out"),
    cancelled: bookings.filter((b) => ["cancelled", "expired"].includes(b.bookingStatus)),
  };
}

export function BookingsList({ bookings }: { bookings: GuestBookingDto[] }) {
  const [tab, setTab] = useState<Tab>("upcoming");
  const [cancelId, setCancelId] = useState<string | null>(null);
  const { showToast } = useToast();
  const router = useRouter();
  const groups = useMemo(() => partition(bookings), [bookings]);

  const tabs: { key: Tab; label: string; data: GuestBookingDto[] }[] = [
    { key: "upcoming", label: "Upcoming", data: groups.upcoming },
    { key: "completed", label: "Completed", data: groups.completed },
    { key: "cancelled", label: "Cancelled", data: groups.cancelled },
  ];

  const active = tabs.find((t) => t.key === tab)!;

  return (
    <div className="section-padding">
      <div className="container-page">
        <h1 className="font-serif text-3xl">My bookings</h1>

        <div className="mt-6 flex gap-2 overflow-x-auto border-b border-border">
          {tabs.map((t) => (
            <button
              key={t.key}
              onClick={() => setTab(t.key)}
              className={`shrink-0 px-4 py-3 text-sm font-medium transition-colors border-b-2 -mb-px ${
                tab === t.key
                  ? "border-foreground text-foreground"
                  : "border-transparent text-muted hover:text-foreground"
              }`}
            >
              {t.label} ({t.data.length})
            </button>
          ))}
        </div>

        {active.data.length === 0 ? (
          <EmptyState
            className="mt-10"
            title={`No ${tab} bookings`}
            description="When you book a stay, it will appear here."
            actionLabel="Explore stays"
            actionHref="/stays"
          />
        ) : (
          <div className="mt-8 space-y-4">
            {active.data.map((booking) => (
              <div
                key={booking.id}
                className="flex flex-col gap-4 rounded-xl border border-border bg-surface p-4 sm:flex-row sm:items-center sm:p-5"
              >
                <div className="relative w-full shrink-0 aspect-[4/3] overflow-hidden rounded-lg sm:w-32 sm:aspect-square">
                  <PropertyThumbnail property={booking.property} className="absolute inset-0 rounded-lg" sizes="128px" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="font-serif text-lg">{booking.property.name}</h3>
                    <BookingStatusBadge status={booking.bookingStatus} />
                  </div>
                  <p className="mt-1 text-sm text-muted">
                    {formatDateRange(booking.checkIn, booking.checkOut)} · {booking.guests} guests
                  </p>
                  <p className="mt-1 text-sm text-muted">#{booking.id}</p>
                  <div className="mt-2 flex items-center gap-3">
                    <span className="font-medium">{formatCurrency(booking.amount)}</span>
                    <PaymentStatusBadge status={booking.paymentStatus} />
                  </div>
                </div>
                <div className="flex w-full flex-col gap-2 sm:w-44">
                  <ButtonLink href={`/bookings/${booking.id}`} variant="outline" size="sm" fullWidth>View booking</ButtonLink>
                  <ButtonLink href="/messages" variant="ghost" size="sm" fullWidth>Message host</ButtonLink>
                  {(booking.bookingStatus === "payment_pending" || booking.bookingStatus === "confirmed") && (
                    <ButtonLink href={`/bookings/${booking.id}#documents`} variant="ghost" size="sm" fullWidth>View / upload ID</ButtonLink>
                  )}
                  {booking.bookingStatus === "confirmed" && (
                    <Button variant="ghost" size="sm" fullWidth className="text-error" onClick={() => setCancelId(booking.id)}>
                      Cancel
                    </Button>
                  )}
                </div>
              </div>
            ))}
          </div>
        )}
      </div>

      <ConfirmDialog
        open={!!cancelId}
        onClose={() => setCancelId(null)}
        danger
        title="Cancel booking?"
        description="This will cancel the booking according to the property policy."
        confirmLabel="Cancel booking"
        onConfirm={async () => {
          if (!cancelId) return;
          const result = await cancelBookingAction(cancelId);
          if (result.ok) {
            showToast("Booking cancelled", "info");
            router.refresh();
          } else {
            showToast(result.error, "error");
          }
          setCancelId(null);
        }}
      />
    </div>
  );
}
